...
import { discountedUnitCents, MAX_TOKENS_PER_ORDER } from '@/lib/team-pricing'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const tier = await userTier(session.userId)
    // Volume tiers stack on whichever base rate this player is on, matching
    // every other buy flow.
    const unitAmount = discountedUnitCents(tier, quantity)
