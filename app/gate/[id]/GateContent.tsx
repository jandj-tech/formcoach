...
function GateContent({ id, region }: { id: string; region: Region }) {
          ...
          {pageState === 'buy-token' && (
            <div className="space-y-6">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6 space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-black font-black text-3xl">
                      $4.99{' '}
                      {currencyLabel && <span className="text-base font-semibold text-gray-500">CAD</span>}
                    </p>
                ...
                >
                  {buying ? 'Redirecting...' : `Buy Analysis — $4.99${currencyLabel}`}
                </button>
              </div>
...
export default function GateContentWrapper({ id, region }: { id: string; region: Region }) {
  return (
    <Suspense fallback={
      <main className="min-h-screen bg-white flex flex-col">
        <TopNav />
      ...
      <GateContent id={id} region={region} />
    </Suspense>
  )
}
