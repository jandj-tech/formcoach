...
const FAQS: Array<{ id?: string; q: string; a: string[] }> = [
  ...
  },
  {
    ...
    a: [
      'Film from the front — camera under or just behind the basket, looking back at the shooter. That is the view that shows the AI what it grades hardest: whether the elbow flares out, whether the guide hand stays passive, whether both hands are heaving the ball into the shot, and whether the feet and shoulders are square. Frame the whole body, head to feet, and keep it that way from the set-up through the release — the stance, the knee bend and which foot is forward are all graded, and a clip cropped at the waist loses them. Not so far away that the player is a small figure across the gym, though: from that distance the elbow and hands are too small to read. One shot per clip.',
