...
UPDATE criteria
SET grading_notes = 'STANCE RUBRIC v18 — measure the base they shot from. That is the score. Then take up to 2 points off if they did not hold it.
...
THERE IS ONE SCORE, NOT TWO SCORES COMBINED. Do NOT score the shot twice and average or weight the results — that is how this criterion goes wrong. Work it out in two steps, and never let the player see the steps.
  STEP A — MEASURE THE BASE BEFORE THE RELEASE. Use the window described under THE BASE below and grade it with the corridor test, the staggered-stance check and the daylight ladder. Whatever that comes to IS the score. This is the base the shot was actually taken on, and it decides the mark.
...
WHERE name = 'Feet Shoulder Width Apart'
  AND (grading_notes IS NULL OR grading_notes NOT LIKE 'STANCE RUBRIC v18%');
